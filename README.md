...
选择协议和端口后会直接开始安装，不再二次确认。如果检测到已有 mosdns、Xray 或 Hysteria，会自动复用或更新。

安装脚本会根据 manifest 动态判断当前状态：未安装任何协议时显示协议菜单；已安装其中一个协议时不显示菜单，直接安装另一个未安装协议；两个协议都已安装时直接退出。
...
卸载时会根据当前安装状态显示可卸载项。两个协议都存在时显示菜单：
...
选择后会直接开始卸载，不再二次确认。只卸载某个协议时默认保留 mosdns。

如果只剩一个协议，卸载脚本不会显示协议菜单，会直接卸载剩余协议。

选择“全部”卸载时，会在安全校验通过后删除 manifest 记录的实际安装目录。只卸载某个协议时不会删除安装目录。

只卸载某个协议时会保留 `reality-mosdns` 命令和安装状态记录，方便继续卸载剩余协议。

组合安装后只卸载其中一个协议时，不会删除另一个协议的服务和配置。

如果只剩最后一个协议，再卸载它时会按“全部卸载”清理安装目录、manifest 和 `reality-mosdns` 命令。
